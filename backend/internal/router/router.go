...
func Setup(cfg *config.Config, db *gorm.DB, redis *util.RedisClient, minio *util.MinIOClient, logger *slog.Logger) *gin.Engine {
	...
	offerRepo := repository.NewHandoverOfferRepository(db)
	apptRepo := repository.NewHandoverAppointmentRepository(db)
	...
	handoverService := service.NewHandoverService(db, offerRepo, apptRepo, appRepo, orgRepo, logger)
	...
	appHandler := handler.NewApplicationHandler(appService, handoverService, logger)
	handoverHandler := handler.NewHandoverHandler(handoverService, logger)
	...
	v1 := r.Group("/api/v1")
	{
		...
		registerApplicationRoutes(v1, cfg, appHandler, handoverHandler, limiter)
		registerHandoverRoutes(v1, cfg, handoverHandler, limiter)
